...
const ImageManager = ({
  ...
  const selectUploadedImage = (image: {
    url: string;
    name: string;
  }) => {
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-hidden animate-fade-in">
        ...
        <Tabs defaultValue="upload" className="w-full">
          ...
          <TabsContent value="upload" className="space-y-4">
            <div className="grid gap-4">
              <div ref={dropZoneRef} className={`border-2 border-dashed rounded-lg p-6 transition-colors ${isDragging ? 'border-primary bg-primary/10' : 'border-border'}`} onDragEnter={handleDragEnter} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onClick={triggerFileInput}>
              ...
              {uploadedImages.length > 0 && <div className="space-y-2">
                  <Label className="text-sm font-medium">Your Images</Label>
                  <ScrollArea className="h-[150px] rounded-md border">
                    <div className="p-4 grid grid-cols-4 gap-2">
                      {uploadedImages.map((image, index) => <div key={index} className={`relative rounded-md overflow-hidden cursor-pointer border-2 ${imageUrl === image.url ? 'border-primary' : 'border-transparent'}`} onClick={() => selectUploadedImage(image)}>
        ...
        <div className="grid gap-4 py-2">
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="altText" value={altText} onChange={e => setAltText(e.target.value)} className="col-span-3 bg-neutral-950" />
          </div>
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <div className="col-span-3">
              <Select value={selectedRatio} onValueChange={value => setSelectedRatio(value as ImageRatio)}>
                ...
                <SelectContent>
                  {sizeOptions.map(option => <SelectItem key={option.id} value={option.id}>{option.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          {selectedRatio === "original" && <>
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="width" type="number" value={width} onChange={e => setWidth(e.target.value)} placeholder="Optional" className="col-span-3 bg-neutral-950" />
              </div>
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input id="height" type="number" value={height} onChange={e => setHeight(e.target.value)} placeholder="Optional" className="col-span-3 bg-neutral-950" />
              </div>
            </>}
        ...
        {imageUrl && <div className="border rounded-md p-4 mt-2">
            ...
            <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded flex justify-center">
              <img src={imageUrl} alt={altText} className={`max-w-full max-h-40 object-contain ${selectedRatio === "1:1" ? "aspect-square" : selectedRatio === "4:3" ? "aspect-4/3" : selectedRatio === "16:9" ? "aspect-video" : ""}`} style={{
